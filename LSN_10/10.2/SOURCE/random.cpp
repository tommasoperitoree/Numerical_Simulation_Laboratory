...
double Random :: Theta(){
   //samples an angle [0, pi/2] utilizing rejection method
   double a = Rannyu(-1., 1.);
   double b = Rannyu(-1., 1.);
   if (b==0){
      return 0;
...
void Random :: RandomImplementation(int rank){
  ...
  if (Primes.is_open())
  {
		for (int i = 0; i < rank; i++) Primes >> p1 >> p2 ; // skip the first rank-1 primes
      Primes >> p1 >> p2;
  }
  else
