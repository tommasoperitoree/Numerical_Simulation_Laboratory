	...
	if(_sim_type == 3) { //Gibbs sampler for Ising
		...
		if(_sim_type == 1){			 // LJ system
			...
			if(this->metro(i)){ //Metropolis acceptance evaluation			
				//_particle(i).setvelocity(0, this->pbc(_particle(i).getposition(0,true) - _particle(i).getposition(0,false), 0)/(2.0 * _delta)); // update velocities
...
bool System :: metro(int i){ // Metropolis algorithm
	...
	if(_rnd.Rannyu() < acceptance ) return true; //Metropolis acceptance step
	else return false;
}
	...
	for (int j=0; j<_npart; j++){
		if(j != i){
			...
			if(dr < _r_cut){
				...
				// cout << "adding energy = " << 1.0/pow(dr,12) - 1.0/pow(dr,6) << " progressive energy = " << energy_i << endl;
			}
		}
	}
	// cout << "___ finished boltzmann evalutation __" << endl;
	return 4.0 * energy_i;
}
...
void System :: initialize(string input_dir){ // Initialize the System object according to the content of the input files in the ../_INPUT/ directory
	...
	while ( !input.eof() ){
		...
		if(property == "SIMULATION_TYPE"){ 
			input >> _sim_type;
			if(_sim_type == 0){	// _sim_type format : SIMULATION_TYPE 0/1 #phase #equilibration
				input >> _int_phase;
			...
			if(_sim_type > 1){ // ising _sim_type = 2/3
				input >> _J;
				input >> _H;
				_equilibration = false;
			}
			...
			if(_sim_type == 0 and _equilibration){
				...
				_output_dir = "../" + _phase +"/MC_NVT/OUTPUT/";
			} else if (_sim_type == 2){
...
void System :: initialize_velocities(){
	...
	for (int i=0; i<_npart; i++){ // print initialized velocities
		// cout << i << " v=(" << _particle(i).getvelocity(0) << "," << _particle(i).getvelocity(1) << "," << _particle(i).getvelocity(2) << ")" << endl;
	}
	return;
}
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
	...
	// cout << "Reading properties completed!" << endl;
	...
	if (_measure_penergy or _measure_pressure or _measure_gofr) {
		for (int i=0; i<_npart-1; i++){
			for (int j=i+1; j<_npart; j++){
				distance(0) = this->pbc( _particle(i).getposition(0,true) - _particle(j).getposition(0,true), 0);
					// cout << "distance x (" << i << "," << j << ")" << endl;
				distance(1) = this->pbc( _particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
					// cout << "distance y (" << i << "," << j << ")" << endl;
				distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
					// cout << "distance z (" << i << "," << j << ")" << endl;
				dr = sqrt( arma::dot(distance,distance) );
					// cout << "calculated dr (" << i << "," << j << ")= " << dr << endl;
				if(dr < _halfside.min() and _measure_gofr){
					// cout << "trying gofr" << endl;
...
void System :: averages(int blk){
	...
	// cout << _block_av(_index_penergy) << endl;
	...
	// GOFR //////////////////////////////////////////////////////////////////////
	if (_measure_gofr and blk==_nblocks and !_equilibration and _sim_type!=2 and _sim_type!=3){
		coutf.open(_output_dir + "gofr.dat",ios::app);
		for(int i=0; i<_n_bins; i++){
